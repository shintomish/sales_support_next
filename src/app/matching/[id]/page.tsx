...
const WORK_STYLE_LABEL: Record<string, string> = { remote: 'リモート', office: '出社', hybrid: 'ハイブリッド' }
...
export default function MatchingPage() {
  ...
  const handleDone = async () => {
    if (proposedCount > 0 && mail) {
      try { await axios.patch(`/api/v1/project-mails/${mail.id}/status`, { status: 'proposed' }) } catch { /**/ }
    }
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-full py-24">
      <p className="text-sm text-gray-500">マッチング計算中...</p>
    </div>
  )
  ...
  return (
    <div className="bg-gray-50 pb-8">
      ...
      <div className="max-w-3xl mx-auto px-4 space-y-4">
        {engineers.length === 0 && (
          <p className="text-center text-gray-400 py-12">マッチする技術者が見つかりません</p>
        )}
        ...
        {RANK_GROUPS.map(g => {
          ...
          return (
            <div key={g.key} className={`rounded-xl border ${g.border} overflow-hidden`}>
              <button onClick={() => setCollapsed(prev => prev.includes(g.key) ? prev.filter(k => k !== g.key) : [...prev, g.key])}
                className={`w-full flex items-center justify-between px-4 py-3 ${g.bg} border-b ${g.border}`}>
                <span className={`text-sm font-bold ${g.color}`}>{g.label} <span className="text-xs text-gray-500 font-normal">{list.length}名</span></span>
                      ...
                      <div key={eng.engineer_id} className={`p-4 ${acted === 'proposed' ? 'bg-green-50' : ''}`}>
                        <div className="flex items-start gap-3">
                          ...
                          <button onClick={() => setModal(eng)}
                            className={`flex-shrink-0 w-12 h-12 rounded-full flex flex-col items-center justify-center border-2 text-white hover:opacity-80 ${eng.score >= 85 ? 'bg-green-500 border-green-600' : eng.score >= 70 ? 'bg-blue-500 border-blue-600' : 'bg-gray-300 border-gray-400'}`}>
                          ...
                          <div className="flex-shrink-0 flex flex-col gap-2">
                            <button onClick={() => setActions(prev => ({ ...prev, [eng.engineer_id]: acted === 'proposed' ? null : 'proposed' }))}
                              className={`text-xs px-3 py-1.5 rounded-lg font-medium w-16 ${acted === 'proposed' ? 'bg-green-600 text-white' : 'bg-green-50 text-green-700 border border-green-300 hover:bg-green-100'}`}>
                              {acted === 'proposed' ? '✔提案' : '提案'}
                            </button>
                            <button onClick={() => setActions(prev => ({ ...prev, [eng.engineer_id]: 'excluded' }))}
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-80 mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="bg-gray-50 border-b px-5 py-4 flex items-center justify-between">
              ...
              <div className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold text-white ${modal.score >= 85 ? 'bg-green-500' : modal.score >= 70 ? 'bg-blue-500' : 'bg-gray-400'}`}>
                {modal.score}
              </div>
            </div>
            <div className="p-5 space-y-3">
              {[
                { label: '① 必須条件', val: modal.breakdown?.requirements ?? 0, max: 40, color: 'bg-green-500' },
                { label: '② スキル',   val: modal.breakdown?.skills       ?? 0, max: 25, color: 'bg-blue-500' },
                { label: '③ 条件',     val: modal.breakdown?.conditions   ?? 0, max: 20, color: 'bg-purple-500' },
                { label: '④ 稼働',     val: modal.breakdown?.availability ?? 0, max: 10, color: 'bg-orange-400' },
                { label: '⑤ 実績',     val: modal.breakdown?.track_record ?? 0, max:  5, color: 'bg-gray-400' },
              ].map(row => (
                <div key={row.label}>
                  ...
                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full ${row.color}`} style={{ width: `${Math.round(row.val / row.max * 100)}%` }} />
                  </div>
                </div>
              ))}
              <div className="border-t pt-3 flex justify-between">
                <span className="text-sm font-semibold text-gray-700">合計</span>
                <span className={`text-xl font-bold ${modal.score >= 85 ? 'text-green-600' : modal.score >= 70 ? 'text-blue-600' : 'text-gray-600'}`}>{modal.score}点</span>
              </div>
            </div>
            <div className="px-5 pb-4 flex justify-end">
              <button onClick={() => setModal(null)} className="text-sm text-gray-500 hover:text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-100">閉じる</button>
            </div>
          </div>
        </div>
      )}
