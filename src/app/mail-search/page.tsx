...
function ResultColumn({ title, kind, res, loading, page, setPage }: {
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col">
      ...
      <div className="flex-1 space-y-2 overflow-y-auto">
        ...
        ) : res.data.map(r => <ResultCard key={`${r.source}-${r.id}`} r={r} />)}
      </div>
      {res && res.last_page > 1 && (
...
export default function MailSearchPage() {
  ...
  const fetchKind = useCallback(async (kind: Kind, page: number) => {
  ...
  const runSearch = useCallback(() => {
    ...
    if (target !== 'engineer') fetchKind('project', 1); else setProjectRes(null);
    if (target !== 'project')  fetchKind('engineer', 1); else setEngineerRes(null);
  }, [target, fetchKind]);
