    ...
    <div className="max-w-3xl mx-auto py-8 px-6">
      <div className="flex justify-between items-start mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">{activity.subject}</h1>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            <div>
              ...
              {activity.customer
                ? <button className="text-sm text-blue-500 hover:underline font-medium"
                    onClick={() => router.push(`/customers/${activity.customer!.id}`)}>{activity.customer.company_name}</button>
                : <Em />}
            </div>
            <div>
              ...
              {activity.contact
                ? <button className="text-sm text-blue-500 hover:underline font-medium"
                    onClick={() => router.push(`/contacts/${activity.contact!.id}`)}>{activity.contact.name}</button>
                : <Em />}
            </div>
            <div>
              ...
              {activity.deal
                ? <button className="text-sm text-blue-500 hover:underline font-medium"
                    onClick={() => router.push(`/deals/${activity.deal!.id}`)}>{activity.deal.title}</button>
                : <Em />}
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-800">{activity.user?.name ?? <Em />}</p>
            </div>
            <div className="col-span-2 md:col-span-3">
              <p className="text-xs text-gray-400 mb-1">件名</p>
              <p className="text-sm font-bold text-gray-800">{activity.subject}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {activity.content && (
        <Card className="shadow-sm">
          <CardHeader className="pb-3"><CardTitle className="text-base text-gray-700">📝 内容</CardTitle></CardHeader>
          <CardContent>
