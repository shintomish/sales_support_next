  ...
  return (
    <div className="flex flex-col h-screen py-4 md:py-8 px-4 md:px-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-wrap justify-between items-center gap-2 mb-4 md:mb-6 flex-shrink-0">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-gray-800">活動履歴</h1>
          ...
          return (
            <div key={a.id}
              ...
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-semibold text-blue-600 flex-1 min-w-0 truncate">{a.subject}</p>
                <span className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold inline-flex items-center gap-0.5 flex-shrink-0"
              ...
              <div className="mt-1 flex items-center justify-between text-xs gap-2 text-gray-500">
                <span className="truncate">{a.deal?.title ?? '—'}</span>
                <span className="whitespace-nowrap flex-shrink-0">📅 {new Date(a.activity_date).toLocaleDateString('ja-JP')}</span>
              </div>
            </div>
          );
        })}
