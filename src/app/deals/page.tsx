...
interface ImportLog {
  ...
  error_details?: { row: number; project_number?: number; reason: string }[];
}
...
function ImportModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  // インポート履歴を取得
  useEffect(() => {
    apiClient.get('/api/v1/deals/import/logs')
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (f && (f.name.endsWith('.xlsx') || f.name.endsWith('.xlsm') || f.name.endsWith('.xls'))) {
      ...
      alert('Excel ファイル（.xlsx / .xlsm）を選択してください');
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await apiClient.post('/api/v1/deals/import', formData, {
  ...
  const statusLabel = (s: string) => s === 'completed' ? '✅ 完了' : s === 'failed' ? '❌ 失敗' : '⏳ 処理中';
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b bg-gray-50">
          <h2 className="text-base font-bold text-gray-800">📥 Excel インポート</h2>
          ...
          {(['upload', 'history'] as const).map(t => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                tab === t ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50/50' : 'text-gray-400 hover:text-gray-600'
              }`}
            >
              {t === 'upload' ? '📤 アップロード' : '📋 インポート履歴'}
            </button>
          ))}
        </div>
          ...
          <div className="p-6 space-y-4">
              ...
              {file ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm font-medium text-gray-600">
                    ここにファイルをドロップ<br />
                    <span className="text-gray-400">または クリックして選択</span>
                  </p>
            ...
            {result && (
              <div className={`rounded-xl p-4 text-sm space-y-2 ${
                ...
                {result.log?.error_details && result.log.error_details.length > 0 && (
                  <details className="mt-2">
                    <summary className="text-xs text-gray-500 cursor-pointer hover:text-gray-700">
                      エラー詳細を確認（{result.log.error_details.length}件）
                    </summary>
                    <div className="mt-2 max-h-32 overflow-y-auto space-y-1">
                      {result.log.error_details.slice(0, 10).map((e, i) => (
        ...
        {tab === 'history' && (
          <div className="p-4">
            {logsLoading ? (
              ...
              <p className="text-center text-sm text-gray-400 py-8">インポート履歴がありません</p>
            ) : (
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {logs.map(log => (
                  <div key={log.id} className="border rounded-lg px-4 py-3 text-sm hover:bg-gray-50">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-medium text-gray-700 truncate max-w-[60%]">{log.original_filename}</span>
                      <span className="text-xs">{statusLabel(log.status)}</span>
                    </div>
                    <div className="flex gap-3 text-xs text-gray-400">
                      <span>総数 {log.total_rows}</span>
                      <span className="text-green-600">新規 {log.created_count}</span>
                      <span className="text-blue-600">更新 {log.updated_count}</span>
                      {log.error_count > 0 && <span className="text-red-500">エラー {log.error_count}</span>}
                    </div>
    ...
    <div className="flex flex-col h-screen py-8 px-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex justify-between items-center mb-6 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" onClick={() => setShowImport(true)} className="gap-1 border-green-300 text-green-700 hover:bg-green-50">
            📥 Excel取込
          </Button>
