...
export default function ProjectMailsPage() {
  ...
  // 右ペインのメモ・備考追記 (手動登録のみ)。POST /project-mails/{id}/memo で本文末尾に追記。
  ...
  const handleAppendMemo = async () => {
    ...
    try {
      const res = await axios.post(`/api/v1/project-mails/${selected.id}/memo`, { body_text: memoText.trim() })
      setSelected(res.data)
      setForm(res.data)
      setMemoText('')
      setShowBody(true)
  ...
  const handleMoveToEngineer = async () => {
    if (!selected) return
    if (!confirm(`「${selected.title || `案件メール #${selected.id}`}」を技術者メールへ移動します。よろしいですか？`)) return
    setMoving(true)
    try {
      await axios.post(`/api/v1/project-mails/${selected.id}/move-to-engineer`)
      setSelected(null)
      setForm({})
      fetchList()
      setSaveMsg({ type: 'ok', text: '技術者メールへ移動しました' })
    } catch {
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="px-5 py-3 bg-gray-50 flex items-center justify-between gap-3 flex-wrap">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  ...
                  <button onClick={handleMoveToEngineer}
                    disabled={moving}
                    title="技術者メールとして誤分類されている場合に移動します"
                    className="text-xs border border-teal-300 text-teal-700 px-3 py-1.5 rounded-lg hover:bg-teal-50 disabled:opacity-50 disabled:cursor-not-allowed">
                    {moving ? '移動中…' : '→ 技術者へ移動'}
                  </button>
                  {sourceMode === 'manual' && (
                    <button
