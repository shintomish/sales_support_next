...
const NAME_BLOCKLIST_RE = /(不可|可否|以上|以下|前後|程度|想定|予算|年収|月収|残業|休日|服装|喫煙|性別|年齢|国籍|学歴|歳|万円|円|ヶ月|カ月|即日|リモート|出社|常駐|経験|スキル|案件|募集|外国籍|日本語|英語|面談|単価|単金|期間|工程|勤務|場所|契約|言語|稼働|必須|歓迎|要員|時給|月給|可能|対応|連絡|確認|検討|大丈夫|同様|予定|別途|本日|明日|宜しく|よろしく|お願|御願|ありがとう|有難う|世話|担当|営業部|採用|参画|交代|交替|延長|新規|急募)/u
...
export function extractRecipientName(body: string | null | undefined): string | null {
  ...
  //   と申します/でございます を優先。です は「カタカナ社名＋漢字氏名＋です」に限定し、
  //   文中の「〜を想定です」等の誤検出を構造的に防ぐ（行末＋直前カタカナ必須）。
  for (const l of lines) {
    const m = l.match(/([一-龯]{2,4})(?:と申します|でございます)/u)
    if (m && isPlausiblePersonName(m[1])) return m[1]
  }
  for (const l of lines) {
    const m = l.match(/[ァ-ヶ]([一-龯]{2,4})です[。.！!\s　]*$/u)
    if (m && isPlausiblePersonName(m[1])) return m[1]
  }
